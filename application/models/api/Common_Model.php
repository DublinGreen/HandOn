...
class Common_model extends CI_Model {
	...
	public function like_data_get($details){
		...
		public function orderByPoroviderId($finalProviderId,$todayDate){
			$sql = $this->db->query("select * from story WHERE userId in ($finalProviderId)  and endDate > '$todayDate' group by userId order by id desc");
			return $sql->result_array();
		}

		public function orderByPoroviderId1($finalProviderId,$todayDate){
			$sql = $this->db->query("select * from liveStory WHERE userId in ($finalProviderId)  and endDate > '$todayDate' group by userId order by id desc");
			return $sql->result_array();
		}
	...
	public function myVideoList($Id){
		...
		//return $sql->result_array();
    	$sql = $this->db->query("SELECT users.username,users.name,users.followerCount as followers,users.image,sounds.title as soundTitle,sounds.id as soundId, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath,userVideos.thumbnail, userVideos.allowComment, userVideos.allowDownloads, userVideos.viewVideo, userVideos.viewCount, userVideos.downloadPath, userVideos.likeCount, userVideos.commentCount,userVideos.downloadPath FROM `userVideos` left join sounds on sounds.id = userVideos.soundId left join users on users.id = userVideos.userId where userVideos.userId = $Id ORDER BY userVideos.id desc");
		return $sql->result_array();
	}

	public function userNotification($userId,$type){
		if(empty($type)){
			...
			$sql = $this->db->query("select notiDate,created FROM userNotification where userId = $userId and type = '$type' group by notiDate order by created desc");
			return $sql->result_array();
		}
	}

  	public function followerUser($userId){
	...
	public function videoSearch($search,$userId){
		...
		$path = base_url();
		if(!empty($search)){
			$sql = $this->db->query("SELECT users.name,users.followerCount as followers, users.username, users.image, users.video, userVideos.*,sounds.title as soundTitle,sounds.id as soundId FROM `userVideos` left join users on users.id = userVideos.userId left join sounds on sounds.id = userVideos.soundId where userVideos.userId NOT IN (select blockUserId from blockUser where userId = $userId ) and userVideos.description LIKE '%$search%' ORDER BY userVideos.viewCount desc,userVideos.likeCount desc,userVideos.commentCount DESC LIMIT 0,10");
			return $sql->result_array();
		}
		else{
	...
	public function gethashTag($search){
		if(!empty($search)){
			$sql = $this->db->query("SELECT * FROM `hashtag` where hashtag LIKE '%$search%' order by videoCount DESC  LIMIT 0,10");
			return $sql->result_array();
		}
		else{
			$sql = $this->db->query("SELECT * FROM `hashtag` order by videoCount DESC  LIMIT 0,10");
			return $sql->result_array();
		}
	}
	...
	public function getCategory($search){
		$path = base_url();
		if(!empty($search)){
			$sql = $this->db->query("SELECT category.*,concat('$path','',category.image) as image FROM `category` where title LIKE '%$search%' order by rand()");
			return $sql->result_array();
		}
		else{
			$sql = $this->db->query("SELECT category.*,concat('$path','',category.image) as image FROM `category` order by rand()");
			return $sql->result_array();
		}
	}
	...
	public function getSoundListApi($search){
		$path = base_url();
		if(!empty($search)){
			$sql = $this->db->query("SELECT sounds.*,concat('$path','',sounds.sound) as sound,concat('$path','',sounds.soundImg) as soundImg  FROM `sounds` where title LIKE '%$search%' order by soundCount DESC  LIMIT 0,10");
			return $sql->result_array();
		}
		else{
			$sql = $this->db->query("SELECT sounds.*,concat('$path','',sounds.sound) as sound,concat('$path','',sounds.soundImg) as soundImg FROM `sounds` order by soundCount DESC  LIMIT 0,10");
			return $sql->result_array();
		}
	}

	public function getHashTagVideos($hashtagIds){
		$sql = $this->db->query("SELECT * FROM `userVideos` where hashTag LIKE '%$hashtagIds%' order by id asc LIMIT 1");
		return $sql->row_array();
