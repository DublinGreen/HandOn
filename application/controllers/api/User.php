...
public function logout()
{
    ...
    if ($this->input->post('type') == 0) {
      $update = $this->db->update('docWorldDoctor', $data, ['id' => $this->input->post('id')]);
    } else {
      $update = $this->db->update('patientdoc', $data, ['id' => $this->input->post('id')]);
    }
  ...
  public function uploadVideosNew(){
		...
		$dir_name = "var/www/html/appCode/application/controllers/api/";
		$uploadPath = "uploads/";
		$duetUrl = "";
		...
		$s3 = new Aws\S3\S3Client([
			...
			'region'  => 'ap-south-1',
			'credentials' => [
					'key'    => 'AKIAI6A7XPIZTQX6INLA',
					'secret' => '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'
			]
	]);
	$bucket = 'tiktokbharat';
		...
		if(!empty($url)){
			$data['videoPath'] = 'http://d2smybu17euu6d.cloudfront.net/'.$_FILES['videoPath']['name'];
		}
		else{
			$data['videoPath'] = '';
		}
		...
		$url = $upload->get('ObjectURL');
		if(!empty($url)){
			$data['downloadPath'] = 'http://d2smybu17euu6d.cloudfront.net/'.basename($downloadPath);
		}
		else{
			$data['downloadPath'] = '';
		}
...
// -fill white    -annotate +5+0 'username'   uploads/anno_splice2.gif
	...
	public function coinList(){
		...
		if(!empty($list)){
			$message['success'] = '1';
			$message['message'] = 'List found Successfully';
			$message['key'] = 'rzp_test_fZaJRVHJJfkIQU';
			foreach($list as $lists){
				if(empty($lists['image'])){
					$lists['image'] = base_url().'uploads/no_image_available.png';
				}
	...
	public function storeLiveBrodcasting(){
		if($this->input->post()){
			...
			curl_setopt($ch, CURLOPT_HTTPHEADER, array(
				...
				'Authorization:Bearer A7BhmiqkYB2zPfvM66wa66',
	...
	public function notification($regId,$message,$type,$loginId,$userId){
		...
		define('API_ACCESS_KEY', 'AAAAX_ZtN8I:APA91bGrgkyC94ecFYVlzjyi2Qf4eukYFOfmFnE23E8zMellShcHuSogo9XheVb4DCCcV2zVHXEMY9jrkFM4no69Q9cnhM3qNa4DXsadGYuF6RqKYU_cfsTRS9o-jVdGDJhI47sNh8Jg');
		 $msg = array(
			 'message' 	=> $message,
			 'title'		=> 'Instahit',
			 'type'		=> $type,
    ...
    if(!empty($this->input->post('channelId'))){
      $checkChannelCount = $this->db->get_where('userChannel',array('id' => $this->input->post('channelId')))->row_array();
      $updateVCount['videoCount'] = $checkChannelCount['videoCount'] + 1;
      $this->Common_Model->update('userChannel',$updateVCount,'id',$this->input->post('channelId'));
    }
...
// 			}
  ...
  public function userCategoryApi($search,$userId){
    ...
    $this->notification($regId,$mess,'message',$sender_id,$reciver_id);
