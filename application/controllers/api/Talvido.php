...
class Talvido extends CI_Controller {
 ...
 public function checkPhoneAndEmail(){
	 ...
	 if(empty($checkData)){
       ...
       $curl = curl_init();
        $phone = $emailPhone;
        $message12 = "Hi, Your Verifaction Code is: ".$otp;
        $a = $phone;
 ...
 public function socialLogin(){
     ...
     if($checkSocialId['status'] == 'Approved'){
       ...
       $message = array(
  			 'success'=>'0',
  			 'message' => 'Your Account has been declined. Please contact support@talvido.com',
  		 );
     }
	 }
 ...
 public function imageVideo(){
	 ...
	 if(!empty($_FILES['image']['name'])){
				 ...
				 if(!empty($url)){
						 $details['image'] = 'http://d2jnk8mn26fih3.cloudfront.net/'.$_FILES['image']['name'];;
				 }
				 else{
						 $details['image'] = '';
				 }
		...
		if(!empty($_FILES['video']['name'])){
			 ...
			 if(!empty($url123)){
					 $details['video'] = 'http://d2jnk8mn26fih3.cloudfront.net/'.$_FILES['video']['name'];;
			 }
			 else{
					 $details['video'] = '';
			 }
 ...
 public function uploadVideosNew(){
  ...
  public function getVideo(){
    ...
    $message['success'] = '0';
    $message['message'] = 'Your Account has been declined. Please contact support@talvido.com';
  }
 	 echo json_encode($message);
  }
  ...
  public function myVideoList(){
    ...
    if(!empty($videoList)){
      foreach($videoList as $lists){
        if($lists['soundTitle'] == null){
            $lists['soundTitle'] = '';
        }
        if($lists['soundId'] == null){
            $lists['soundId'] = '';
        }
  ...
  public function userInfo(){
    ...
    if(empty($getUserDetails) || $getUserDetails['status'] == 'pending'){
      ...
      $finalData['sendBeans'] = '100';
      $finalData['usernameStatus'] = '0';
    ...
    public function userCategoryApi($search,$userId){
     ...
     public function viewVideo(){
       ...
       if($this->input->post('type') == 1){
        ...
        $data1['userViewCount'] = $getCount['userViewCount'] +  1;
        $update = $this->Common_Model->update('adminVideos',$data1,'id',$this->input->post('videoId'));
        if($getCount['viewCount'] == $data1['userViewCount']){
          $data12['status'] = 'Expired';
          $update = $this->Common_Model->update('adminVideos',$data12,'id',$this->input->post('videoId'));
        }
        $message['success'] = '1';
        $message['message'] = 'Count update successfully';
       }
       ...
       $mess = $loginUserDetails['username'].' send you a message';
    	 ...
    	 if(empty($get_sender_data)){
          ...
          $bucket = 'zebovideos';
    			 ...
    			 $url = $upload->get('ObjectURL');
    			 if(!empty($url)){
    				 $data['image'] = 'http://d2jnk8mn26fih3.cloudfront.net/'.$_FILES['image']['name'];
    			 }
    			 else{
    				 $data['image'] = '';
    			 }
    			 $sendResponse['image'] = 'http://d2jnk8mn26fih3.cloudfront.net/'.$_FILES['image']['name'];
    		 }
    		 $data['created'] = date('y-m-d H:i:s');
     ...
     public function sendGift(){
      ...
      public function categoryVideos(){
        ...
        $list =  $this->db->query("SELECT category.title as categoryTitle, sounds.title as soundTitle,sounds.id as soundId,sounds.soundCount as videoCount,sounds.sound as soundPath,sounds.type as soundType, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath, userVideos.allowComment, userVideos.allowDownloads,userVideos.viewVideo,userVideos.viewCount,userVideos.likeCount, userVideos.commentCount,userVideos.viewCount,userVideos.allowDuetReact,userVideos.downloadPath FROM `userVideos` left join sounds on sounds.id = userVideos.soundId left join category on category.id = userVideos.categoryId where userVideos.categoryId = $categoryId and userVideos.userId NOT IN (select blockUserId from blockUser where userId = $userId  ) ORDER BY userVideos.id ASC LIMIT  0 , 10")->result_array();
      ...
      public function home(){
        ...
        if($this->input->post('type') == 'video'){
           $list =  $this->db->query("SELECT users.username,users.name,users.followerCount as followers,users.image,sounds.title as soundTitle,sounds.id as soundId, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath, userVideos.allowComment, userVideos.allowDownloads,userVideos.viewVideo,userVideos.likeCount,userVideos.commentCount,userVideos.viewCount,userVideos.downloadPath FROM `userVideos` left join sounds on sounds.id = userVideos.soundId left join users on users.id = userVideos.userId where userVideos.userId NOT IN (select blockUserId from blockUser where userId = '$userId' ) and userVideos.id NOT IN (select videoId from  viewVideo where userId = '$userId' ) ORDER BY userVideos.viewCount desc,userVideos.likeCount desc,userVideos.commentCount LIMIT $startLimit , 10")->result_array();
        }
        elseif($this->input->post('type') == 'following'){
          ...
          if(!empty($follwerList)){
            ...
            $fIds = implode(',',$idList);
           $list =  $this->db->query("SELECT users.username,users.name,users.followerCount as followers,users.image,sounds.title as soundTitle,sounds.id as soundId, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath, userVideos.allowComment, userVideos.allowDownloads,userVideos.viewVideo,userVideos.likeCount,userVideos.commentCount,userVideos.viewCount,userVideos.downloadPath FROM `userVideos` left join sounds on sounds.id = userVideos.soundId left join users on users.id = userVideos.userId where userVideos.userId  IN ($fIds ) and userVideos.id NOT IN (select videoId from  viewVideo where userId = '$userId' ) ORDER BY userVideos.viewCount desc,userVideos.likeCount desc,userVideos.commentCount LIMIT $startLimit , 10")->result_array();
         }
         else{
           $list = '';
         }
        ...
        if(!empty($list)){
          ...
          $message['message'] = 'List Found Successfully';
          foreach($list as $lists){
            if(empty($lists['soundTitle']) || $lists['soundTitle'] == ''){
              $lists['soundTitle'] = '';
              $lists['soundId'] = '';
            }
            else{
              $lists['soundTitle'] =  $lists['soundTitle'];
              $lists['soundId'] = $lists['soundId'];
            }
     ...
     public function agoraToken(){
       ...
       if(empty($checkUser)){
         ...
         $upLiveData['archivedDate'] = date('Y-m-d H:i:s');
         $upLiveData['status'] = 'archived';
         $this->Common_Model->update('userLive',$upLiveData,'userId',$this->input->post('userId'));

         require APPPATH.'/libraries/agora/RtcTokenBuilder.php';
           require APPPATH.'/libraries/agora/RtmTokenBuilder.php';
         $appID = "fb67eab7f492425fa5f777cff48bdceb";
         $appCertificate = "36f882d6d0454a0fb2e26f217928e8ca";
         ...
         if(!empty($token)){
           ...
           $data['latitude'] = $this->input->post('latitude');
           $data['longitude'] = $this->input->post('longitude');
           if(!empty($this->input->post('hostType'))){
...
// -fill white    -annotate +5+0 'username'   uploads/anno_splice2.gif
 ...
 function addtextImage($gifpath,$watermarkpath,$textwrite){
  ...
  // die;
   ...
   // Annotate a text
   $gmagick->annotateImage($draw,25,75 ,0, $textwrite);
...
// 			}
 ...
 public function hashTagName($ids){
  ...
  public function videoTesting(){
    ...
    $endLimit = 9;
    if($this->input->post('type') == 'video'){
 	    $list =  $this->db->query("SELECT users.username,users.name,users.followerCount as followers,users.image,sounds.title as soundTitle,sounds.id as soundId, userVideos.id, userVideos.userId, userVideos.hashtag, userVideos.description, userVideos.videoPath, userVideos.allowComment, userVideos.allowDownloads,userVideos.viewVideo,userVideos.likeCount,userVideos.commentCount,userVideos.viewCount,userVideos.downloadPath FROM `userVideos` left join sounds on sounds.id = userVideos.soundId left join users on users.id = userVideos.userId where userVideos.userId NOT IN (select blockUserId from blockUser where userId = '$userId' ) and userVideos.id NOT IN (select videoId from  viewVideo where userId = '$userId' ) ORDER BY userVideos.viewCount desc,userVideos.likeCount desc,userVideos.commentCount LIMIT $startLimit , 9")->result_array();
    }
    elseif($this->input->post('type') == 'following'){
      $follwerList = $this->db->get_where('userFollow',array('userId' => $this->input->post('userId'),'status' => '1'))->result_array();
 		 if(!empty($follwerList)){
     ...
     $adminVideo =  $this->db->query("select * from adminVideos where status = 'Approved' ORDER BY RAND() LIMIT 1")->row_array();
     if(!empty($adminVideo)){
       $adminVideo['advertisementStatus'] = '1';
       $adminVideo['viewCount'] = $adminVideo['userViewCount'];

       if(!empty($adminVideo['buttonName'])){
         $userInfo = $this->db->get_where('users',array('username' => $adminVideo['buttonName']))->row_array();
         //die;
         if(!empty($userInfo)){
           $adminVideo['username'] = $userInfo['username'];
           $adminVideo['image'] = $userInfo['username'];
           if(empty($userInfo['image'])){
        		 $adminVideo['image'] = base_url().'uploads/no_image_available.png';
        	 }
      		 else{
      			$adminVideo['image'] = $userInfo['image'];
      		 }
     				$adminVideo['likeStatus'] = true;

     			 $checkFollow = $this->db->get_where('userFollow', array('userId' => $this->input->post('userId'),'followingUserId' =>$userInfo['id'],'status' => '1'))->row_array();
     			 if(!empty($checkFollow)){
     				 $adminVideo['followStatus'] = '1';
     			 }
     			 else{
     				 $adminVideo['followStatus'] = '0';
     			 }

           $followersCount = $this->db->get_where('userFollow', array('followingUserId' =>$userInfo['id'],'status' => '1'))->num_rows();
     			 if(!empty($followersCount)){
     				 $adminVideo['followers'] = (string)$followersCount;
     			 }
     			 else{
     				 $adminVideo['followers'] = '0';
     			 }
           $adminVideo['userId'] = $userInfo['id'];
         }
         else{
           $adminVideo['username'] = '';
           $adminVideo['userId'] = '';
	...
	public function liveHistory(){
		$userId = $this->input->post('userId');
		$liveHistory = $this->db->query("SELECT id,userId,channelName,token,status,created FROM `userLive` where userId = $userId group by date(created) order by created desc")->result_array();
		if(!empty($liveHistory)){
			$message['success'] = '1';
			$message['message'] = 'List found successfully';
			foreach($liveHistory as $list){
				//$liveDate = $list['created'];
				$date11=date_create($list['created']);
				$dateTitle =  date_format($date11,"Y-m-d");
				$dateWiseData = $this->db->query("SELECT * FROM `userLive` where userId = $userId and date(created) = '$dateTitle'")->result_array();
				if(!empty($dateWiseData)){
					$finalMin = ' ';
					foreach($dateWiseData as $dateWiseDatas){
						if(!empty($dateWiseDatas['archivedDate']) || $dateWiseDatas['archivedDate'] != ''){
							$date1 = new DateTime($dateWiseDatas['created']);
							$date2 = $date1->diff(new DateTime($dateWiseDatas['archivedDate']));
							$dayHour =  $date2->d * 60;
							$totalHour = $date2->h * 60;
						  $finalMin += $date2->i + $totalHour + $dayHour;
						}
					}
					$time = $finalMin;
					if(!empty($time) || $time != ' ' ){
						$list['totalTime'] = (string)$time;
					}
					else{
						$list['totalTime'] = "00";
					}
				...
				$mainId = $list['id'];
				$countGiftCoin =  $this->db->query("SELECT sum(coin) as totalCoin FROM `userGiftHistory` where liveId = $mainId")->row_array();
				if(!empty($countGiftCoin['totalCoin'])){
					$list['totalCoin'] = $countGiftCoin['totalCoin'];
				}
				else{
					$list['totalCoin'] = '0';
				}
  ...
  public function sliderList(){
    $list = $this->db->order_by('id','desc')->get_where('slider',array('status' => 'Approved'))->result_array();
    if(!empty($list)){
      $message['success'] = '1';
      $message['message'] = 'list found succssfully';
      foreach($list as $lists){
