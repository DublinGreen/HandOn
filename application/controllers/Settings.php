...
class Settings extends CI_Controller {
	...
	public function splash(){
		...
		$data['details'] = $this->db->get_where('logo','id=3')->row_array();
		$data['active'] = 'splash';
		$data['title'] = 'Manage Splash Image';
		if($this->input->post()){
      ...
      if (empty($_FILES['image']['name'])){
    		...
    		$this->load->view('admin/settings/splash');
        ...
        $details['updated'] = date('Y-m-d H:i:s');
				...
				$update = $this->db->update('logo',$details,['id' => 3]);

				if($update){
					$this->session->set_flashdata('success', "Splash Image Updated Successfully");
					redirect(site_url().'/Settings/splash');
				}
      }
		}
		else{
