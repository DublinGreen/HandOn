...
class Badges extends CI_Controller {
		...
		if($this->input->post()){
			...
			if($this->form_validation->run() == FALSE){
				...
				$s3 = new Aws\S3\S3Client([
						...
						'region'  => 'ap-south-1',
						'credentials' => [
								'key'    => 'AKIA4BYPQOBYPZD2HFX2',
								'secret' => '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'
						]
				]);
				$bucket = 'rockstarvideo';
	...
	public function manageCrown(){
			$admin_details = $this->session->userdata('admin_details');
			$data['admin'] = $this->Common_model->get_data_by_id('admin','id',$admin_details['admin_id']);
			$data['details'] = $this->db->get_where('crown')->result_array();
			$data['active'] = 'crown';
			$data['title'] = 'Manage Crown';
			$this->load->view('admin/includes/header',$data);
			$this->load->view('admin/badges/manageCrown');
			$this->load->view('admin/includes/footer');
	}
public function addCrown(){
			...
			if($this->input->post()){
					...
					if($this->form_validation->run() == FALSE){
							$this->load->view('admin/includes/header',$data);
							$this->load->view('admin/badges/addCrown');
							$this->load->view('admin/includes/footer');
					}
					else{
							require APPPATH.'/libraries/vendor/autoload.php';
							...
							'region'  => 'eu-west-2',
							'credentials' => [
								'key'    => 'AKIAZQNEMALGRLH2QQPD',
								'secret' => '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'
							]
						]);
						$bucket = 'abidsvideo';
							...
							$details['totalFollowers'] = $this->input->post('totalFollowers');
							$details['color'] = $this->input->post('color');
							$insert = $this->db->insert('crown',$details);
							if($insert){
									$this->session->set_flashdata('success', "Crown Added Successfully");
									redirect(site_url().'/Badges/manageCrown');
							}
					}
			}
	...
	public function editCrown(){
			...
			if($this->input->post()){
					...
					if($this->form_validation->run() == FALSE){
							$this->load->view('admin/includes/header',$data);
							$this->load->view('admin/badges/editCrown');
							$this->load->view('admin/includes/footer');
					}
					else {
							// require APPPATH . '/libraries/vendor/autoload.php';
						...
						$bucket = 'abidsvideo';
							...
							$update = $this->Common_model->update('crown',$details,'id',$this->uri->segment(3));
							if($update){
									$this->session->set_flashdata('success', "Crown Updated Successfully");
									redirect(site_url().'/Badges/manageCrown');
							}
					...
					$data['details'] = $this->db->get_where('crown',array('id' => $this->uri->segment(3)))->row_array();
					$this->load->view('admin/includes/header',$data);
					$this->load->view('admin/badges/editCrown');
					$this->load->view('admin/includes/footer');
			}
	...
	public function deleteCrown(){
			$delete = $this->Common_model->delete('crown','id',$this->uri->segment(3));
			if($delete) {
					$this->session->set_flashdata('success',"Crown deleted Successfully");
					redirect(site_url() . '/Badges/manageCrown');
			}
	}
